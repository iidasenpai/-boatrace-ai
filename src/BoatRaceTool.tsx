...
function getBetFlowLabel(combo, boats) {
  const lanes = String(combo).split('-').map(Number);
  const first = boats.find(b => b.lane === lanes[0]);
  const second = boats.find(b => b.lane === lanes[1]);
  if (!first) return '展開不明';

  const course = Number(first.entryCourse || first.lane);
  const fastStart = first.exST != null && first.exST <= 0.08;
  const strongGear = Number(first.gearScore || 0) >= 55;

  if (course === 1) return second && Number(second.entryCourse || second.lane) >= 4 ? '逃げ＋外伸び' : '逃げ本線';
  if (course === 2) return fastStart || strongGear ? '差し本線' : '差し粘り';
  if (course === 3) return fastStart ? 'まくり差し' : '差し一撃';
  if (course === 4) return strongGear ? 'カドまくり' : 'まくり差し';
  return strongGear ? '外伸び一撃' : '高配当展開';
}

function getFlowSuitability(combo, boats) {
  const lanes = String(combo).split('-').map(Number);
  const selected = lanes.map(lane => boats.find(b => b.lane === lane)).filter(Boolean);
  if (selected.length !== 3) return 50;

  const totals = boats.map(b => Number(b.total || 0));
  const maxTotal = Math.max(...totals, 1);
  const power = selected.reduce((sum, b, index) => {
    const weight = index === 0 ? 0.5 : index === 1 ? 0.3 : 0.2;
    return sum + (Number(b.total || 0) / maxTotal) * 100 * weight;
  }, 0);

  const first = selected[0];
  const course = Number(first.entryCourse || first.lane);
  let courseBonus = 0;
  if (course === 1) courseBonus = 8;
...
function getValueScore(bet) {
  if (bet.expectedValue !== null && bet.expectedValue !== undefined) {
    return Math.round(clampScore((bet.expectedValue - 70) * 1.7));
  }
  if (bet.returnExpectationScore !== null && bet.returnExpectationScore !== undefined) {
    return Math.round(clampScore(Number(bet.returnExpectationScore) / 1.35));
  }
  return 45;
}
...
function getRaceDecision(grade, metrics) {
  const battle = Number(metrics?.battle || 0);
  const upset = Number(metrics?.upset || 0);
  const recommendation = Number(grade?.recommendation || 0);
  if (grade?.grade === 'S+' && battle >= 88 && upset <= 30) return { label: '鉄板', color: '#52d273', icon: '🟢', action: '厚め購入候補' };
  if ((grade?.grade === 'S' || grade?.grade === 'A') && recommendation >= 74 && upset <= 55) return { label: '本命', color: '#52d273', icon: '🟢', action: '購入候補' };
  if (recommendation >= 62 && upset <= 68) return { label: '混戦', color: '#e8b800', icon: '🟡', action: '点数を絞る' };
  if (upset >= 72 && recommendation >= 48) return { label: '波乱', color: '#ff9b6b', icon: '🟠', action: '少額で狙う' };
  return { label: '見送り', color: '#ff6b6b', icon: '🔴', action: '無理に買わない' };
}

function getTopAxisLabel(boats) {
  if (!Array.isArray(boats) || boats.length < 2) return '';
  const sorted = [...boats].sort((a,b) => Number(b.total||0)-Number(a.total||0));
  const gap = Number(sorted[0].total||0)-Number(sorted[1].total||0);
  return gap <= 1.5 ? `W本命 ${sorted[0].lane}号艇・${sorted[1].lane}号艇` : `軸 ${sorted[0].lane}号艇`;
}

function buildAnaConditions(anaBoat, boats) {
  if (!anaBoat || !Array.isArray(boats)) return [];
  const top = [...boats].sort((a,b) => Number(b.total||0)-Number(a.total||0))[0];
  const conditions = [];
  const course = Number(anaBoat.entryCourse || anaBoat.lane);
  if (top && Number(top.entryCourse || top.lane) === 1) conditions.push(`${top.lane}号艇のインが流れる`);
  if (course >= 3 && course <= 4) conditions.push(`${anaBoat.lane}号艇が先に攻める`);
  else if (course >= 5) conditions.push(`${anaBoat.lane}号艇が外から伸びる`);
  else conditions.push(`${anaBoat.lane}号艇が差し場を突く`);
  if (Number(anaBoat.exScore||0) >= 68) conditions.push('展示気配どおりに走る');
  else if (Number(anaBoat.stScore||0) >= 68) conditions.push('スタートを決める');
  else if (Number(anaBoat.gearScore||0) >= 62) conditions.push('機力を生かして連に絡む');
  return conditions.slice(0,3);
}

function rankAndAllocateBets(bets, boats, budgetYen) {
  ...
  // 最低100円を確保したうえで、最終評価の「点差」に応じて残り予算を配分する。
  // 近い評価なら均等、明確な1位なら厚くするため、絶対値ではなく首位との差を使う。
  const topScore = Number(active[0]?.finalScore || 0);
  const weights = active.map((b, i) => {
    const gap = Math.max(0, topScore - Number(b.finalScore || 0));
    const relative = Math.exp(-gap / 3.2);
    return Math.max(0.05, relative * (i === 0 ? 1.22 : 1));
  });
  ...
  while (remaining > 0) {
    ...
    for (let i = 0; i < active.length; i++) {
      const target = (units * weights[i]) / weightSum;
      const need = target - alloc[i];
  ...
  return active.map((bet, i) => ({
    ...bet,
    yen: alloc[i] * 100,
    allocationRate: Math.round((alloc[i] / units) * 100),
  }));
}
...
function buildLearningSummary(races) {
  ...
  const recentMoney = recent.filter(r => Number(r.stakeYen) > 0 || Number(r.payoutYen) > 0);
  ...
  const recentStake = recentMoney.reduce((sum, r) => sum + Math.max(0, Number(r.stakeYen) || 0), 0);
  const recentPayout = recentMoney.reduce((sum, r) => sum + Math.max(0, Number(r.payoutYen) || 0), 0);
  const roi = stake > 0 ? Math.round((payout / stake) * 1000) / 10 : null;
  const recentRoi = recentStake > 0 ? Math.round((recentPayout / recentStake) * 1000) / 10 : null;
  ...
  const rankErrors = completed.map(r => {
    const lanes = parseResultLanes(r.result);
    const sorted = Array.isArray(r.boats) ? [...r.boats].sort((a,b)=>Number(b.total||0)-Number(a.total||0)) : [];
    if (!lanes?.[0] || !sorted.length) return null;
    const predictedRank = sorted.findIndex(b => b.lane === lanes[0]) + 1;
    return predictedRank > 0 ? Math.abs(predictedRank - 1) : null;
  }).filter(v => v != null);
  const avgRankError = rankErrors.length ? Math.round(rankErrors.reduce((a,b)=>a+b,0)/rankErrors.length*10)/10 : null;
  const inRaces = completed.filter(r => {
    const best = Array.isArray(r.boats) ? [...r.boats].sort((a,b)=>Number(b.total||0)-Number(a.total||0))[0] : null;
    return Number(best?.entryCourse || best?.lane) === 1;
  });
  const outerRaces = completed.filter(r => {
    const best = Array.isArray(r.boats) ? [...r.boats].sort((a,b)=>Number(b.total||0)-Number(a.total||0))[0] : null;
    return Number(best?.entryCourse || best?.lane) >= 4;
  });
  const patternRate = list => list.length ? list.filter(r => {
    const lanes=parseResultLanes(r.result); const best=[...(r.boats||[])].sort((a,b)=>Number(b.total||0)-Number(a.total||0))[0];
    return lanes?.[0]===best?.lane;
  }).length/list.length*100 : null;
  const inRate=patternRate(inRaces), outerRate=patternRate(outerRaces);
  let strength='データ蓄積中', weakness='データ蓄積中';
  if (completed.length >= 5) {
    if (inRate != null && (outerRate == null || inRate >= outerRate)) { strength='イン本命戦'; weakness=outerRate != null ? '外枠軸戦' : '混戦'; }
    else { strength='外枠軸戦'; weakness='イン本命戦'; }
  }
  return {
    total: completed.length, recentCount: recent.length,
    hitRate: trifecta.betsHitRate, recentHitRate: recentStats.betsHitRate,
    straightRate: trifecta.straightRate, boxRate: trifecta.boxRate,
    roi, recentRoi, profit, stake, payout, moneyRaceCount: moneyRaces.length,
    topPickWinRate: completed.length ? Math.round(firstPickWins / completed.length * 1000) / 10 : null,
    avgRankError, strength, weakness,
  };
}
...
export default function BoatRaceTool() {
  ...
  const [imageProgress, setImageProgress] = useState({ phase: 'idle', percent: 0, label: '' });
    ...
    try {
      const result = await callVisionAPI(images, info => {
        if (info.phase === 'connecting') {
          ...
          setImageProgress({ phase: 'complete', percent: 94, label: '予想と買い目を生成しています' });
        }
      });
...
setForecast(generateForecast(scored));
      ...
      setImageCanRetry(false);
      setServerState('available');
      setImageProgress({ phase: 'done', percent: 100, label: '解析が完了しました' });
    } catch (e) {
  ...
  const raceDecision = getRaceDecision(raceGrade, raceMetrics);
  const topAxisLabel = getTopAxisLabel(boats);
  const anaConditions = buildAnaConditions(anaResult?.anaBoat, boats);
        ...
        <div style={{ marginTop: 10, marginBottom: 16, borderTop: '1px solid #1c2b3d', paddingTop: 14 }}>
          ...
          {(imageLoading || imageProgress.phase === 'done') && (
            <div style={{ marginTop: 10, border: '1px solid #2a3d52', background: '#101b29', borderRadius: 8, padding: 10 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, fontSize: 11, marginBottom: 7 }}>
                <span style={{ color: '#dce8f5', fontWeight: 700 }}>{imageProgress.label}</span>
                <span style={{ color: '#8ba3bd' }}>{imageProgress.percent}%</span>
              </div>
              <div style={{ height: 7, background: '#07111d', borderRadius: 999, overflow: 'hidden' }}>
        ...
        {boats && raceSummary && (
          <div style={{ marginBottom: 18 }}>
            <div style={{
              ...
              <div style={{ marginTop: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#0f1a28', border: `1px solid ${raceDecision.color}`, borderRadius: 8, padding: '9px 12px' }}>
                <div><div style={{ fontSize: 10, color: '#8ba3bd' }}>最終判断</div><div style={{ fontSize: 19, fontWeight: 900, color: raceDecision.color }}>{raceDecision.icon} {raceDecision.label}</div></div>
                <div style={{ textAlign: 'right' }}><div style={{ fontSize: 10, color: '#8ba3bd' }}>{topAxisLabel}</div><div style={{ fontSize: 13, fontWeight: 900, color: raceDecision.color }}>{raceDecision.action}</div></div>
              </div>
              ...
              {bets && (
                <div>
                  {bets.length > 0 && (
                    <div
                      style={{
                        background: 'linear-gradient(135deg, #10251d, #132234)',
                        border: '1px solid #2f9b5b',
                        borderRadius: 9,
                        padding: '12px 14px',
                        marginBottom: 10,
                      }}
                    >
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'end', gap: 12, marginTop: 4 }}>
                        ...
                        <div style={{ textAlign: 'right' }}>
                          <div style={{ color: '#e8b800', fontSize: 20, fontWeight: 900 }}>{bets[0].yen}円</div>
                          <div style={{ color: '#8ba3bd', fontSize: 10 }}>予算の{bets[0].allocationRate}%</div>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {bets.slice(1).map((b, i) => {
                    ...
                    return (
                      ...
                      >
                        <div>
                          <div style={{ fontSize: 17, fontWeight: 800, color: '#f2f6fa' }}>{b.combo}</div>
                          <div style={{ marginTop: 5, fontSize: 10, color: '#8ba3bd' }}>{b.flowLabel}</div>
                        </div>
                        ...
                        <div style={{ minWidth: 0 }}>
                          ...
                          {b.odds !== null && b.odds !== undefined ? (
                            <div style={{ fontSize: 11, marginTop: 4, color: b.expectedValue >= 100 ? '#7aff9b' : '#ff8080' }}>
                              オッズ {b.odds}倍 ／ 期待値 {b.expectedValue}% ／ {b.evLabel}
                            </div>
                          ) : (
                            <div style={{ fontSize: 10, color: '#8ba3bd', marginTop: 4 }}>個別オッズ未取得</div>
                          )}
                            ...
                            <div style={{ marginTop: 5 }}>
                              ...
                              <div style={{ height: 5, background: '#0b1420', borderRadius: 999, marginTop: 3, overflow: 'hidden' }}>
                                <div
                                  style={{
                                    height: '100%',
                                    width: `${Math.max(4, Math.min(100, Number(b.returnExpectationScore) || 0))}%`,
                        ...
                        <div style={{ textAlign: 'right', flexShrink: 0 }}>
                          <div style={{ fontWeight: 800, fontSize: 16, color: '#e8b800' }}>{b.yen}円</div>
                          <div style={{ fontSize: 10, color: '#8ba3bd', marginTop: 3 }}>配分 {b.allocationRate}%</div>
                        </div>
                      </div>
                    );
                  })}
  ...
  合計 {bets.reduce((a, c) => a + c.yen, 0)}円
            ...
            <div style={{ marginTop: 20, borderTop: '1px solid #1c2b3d', paddingTop: 14 }}>
              ...
              {anaResult && anaResult.anaBoat && (
                <div>
                  ...
                  {anaConditions.length > 0 && <div style={{ background:'#0f1a28', borderRadius:8, padding:'8px 10px', marginBottom:8 }}>
                    <div style={{ fontSize:10, color:'#7aa6e8', fontWeight:900, marginBottom:4 }}>穴成立条件</div>
                    {anaConditions.map((condition, i) => <div key={condition} style={{ fontSize:11, color:'#c5d3e0', lineHeight:1.7 }}>{i+1}. {condition}</div>)}
                  </div>}
        ...
        <div style={{ marginTop: 12, background: '#111d2b', border: '1px solid #2a3d52', borderRadius: 10, padding: 14 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8 }}>
            ...
            <div style={{ background: '#0f1a28', borderRadius: 8, padding: 9 }}><div style={{ fontSize: 10, color: '#8ba3bd' }}>直近20R的中率</div><div style={{ fontSize: 18, fontWeight: 900, color: '#52d273' }}>{learningSummary.recentHitRate ?? '-'}{learningSummary.recentHitRate != null ? '%' : ''}</div><div style={{ fontSize: 9, color: '#71869b' }}>{learningSummary.recentCount}件</div></div>
            <div style={{ background: '#0f1a28', borderRadius: 8, padding: 9 }}><div style={{ fontSize: 10, color: '#8ba3bd' }}>直近20R回収率</div><div style={{ fontSize: 18, fontWeight: 900, color: learningSummary.recentRoi != null && learningSummary.recentRoi >= 100 ? '#52d273' : '#ff9b6b' }}>{learningSummary.recentRoi ?? '-'}{learningSummary.recentRoi != null ? '%' : ''}</div></div>
            <div style={{ background: '#0f1a28', borderRadius: 8, padding: 9 }}><div style={{ fontSize: 10, color: '#8ba3bd' }}>平均順位誤差</div><div style={{ fontSize: 18, fontWeight: 900, color: '#7aa6e8' }}>{learningSummary.avgRankError ?? '-'}</div><div style={{ fontSize: 9, color: '#71869b' }}>小さいほど良好</div></div>
            <div style={{ background: '#0f1a28', borderRadius: 8, padding: 9 }}><div style={{ fontSize: 10, color: '#8ba3bd' }}>得意 / 苦手</div><div style={{ fontSize: 12, fontWeight: 900, color: '#52d273' }}>{learningSummary.strength}</div><div style={{ fontSize: 10, color: '#ff9b6b', marginTop:3 }}>{learningSummary.weakness}</div></div>
          </div>
        </div>
