...
function rankAndAllocateBets(bets, boats, budgetYen) {
  ...
  const fractions = weights.map((w, i) => ({ i, raw: remaining * w / weightSum }));
  fractions.forEach(({ i, raw }) => { const whole = Math.floor(raw); alloc[i] += whole; remaining -= whole; });
  fractions.sort((a, b) => (b.raw % 1) - (a.raw % 1));
  for (let i = 0; i < remaining; i++) alloc[fractions[i % fractions.length].i] += 1;
